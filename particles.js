...
class Emitter {
    ...
    constructor(position, velocity, spread, size, color, particleSize, emissionRate, maxParticles, lifespan) {
        this.position = position;
        this.velocity = velocity;
        this.spread = spread;
        this.size = size;
    ...
    addNewParticles(step) {
        ...
        for (var j = 0; j < particlesToEmmit; j++) {
            ...
            var segment = Vector.fromAngle(this.velocity.getAngle() + Math.PI / 2, this.size);
            var randomSegment = Vector.fromAngle(this.velocity.getAngle() - Math.PI / 2, Math.random() * this.size * 2);
            segment.add(randomSegment);
            segment.add(this.position);
            var position = segment;
            var velocity = Vector.fromAngle(angle, magnitude);
            var particle = new Particle(position, velocity, new Vector(0, 0), this.color, this.particleSize, this.lifespan);
